...
self.onmessage = function(e) {
  ...
  case "command":
    ...
    postMessage({ id: id, status: ret.value, reply: reply });
    break;
  case "wait_for_event":
    var ret = libhardware_legacy.wait_for_event(cbuf, 4096);
    var event = cbuf.readString().substr(ret.value);
  ...
  case "dhcp_release_lease":
    ...
    postMessage({ id: id, status: ret.value });
    break;
  ...
  case "dhcp_do_request_renew":
    ...
    postMessage({ id: id, status: ret.value, ipaddr: ints[0], gateway: ints[1], mask: ints[2],
                  dns1: ints[3], dns2: ints[4], server: ints[5], lease: ints[6]});
    break;
  case "property_get":
    var ret = libcutils.property_get(data.key, cbuf, data.defaultValue);
    postMessage({ id: id, status: ret.value, value: cbuf.readString() });
    break;
  case "property_set":
